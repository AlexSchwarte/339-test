...
import { endpoint as _endpoint, key as _key, database as _database, container as _container, items } from './config'
import url from 'url'

const endpoint = _endpoint
const key = _key

const databaseId = _database.id
...
createDatabase()
  ...
  .then(() => readContainer())
  .then(() => scaleContainer())
  .then(() => createFamilyItem(items.Andersen))
  .then(() => createFamilyItem(items.Wakefield))
  .then(() => queryContainer())
  .then(() => replaceFamilyItem(items.Andersen))
  .then(() => queryContainer())
  .then(() => deleteFamilyItem(items.Andersen))
  .then(() => {
